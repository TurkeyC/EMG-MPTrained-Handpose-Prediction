...
# Convert INFOLKS JSON file into YOLO-format Labels_Data ----------------------------
def convert_infolks_json(name, files, img_path):
    """Converts INFOLKS JSON annotations to YOLO-format Labels_Data."""
    path = make_dirs()
    ...
    for x in tqdm(data, desc="Files and Shapes"):
        ...
        cat.extend(a["classTitle"].lower() for a in x["Model_Output"]["objects"])  # categories
    ...
    # Write Labels_Data file
    for i, x in enumerate(tqdm(data, desc="Annotations")):
        label_name = Path(file_name[i]).stem + ".txt"

        with open(path + "/Labels_Data/" + label_name, "a") as file:
            for a in x["Model_Output"]["objects"]:
...
# Convert vott JSON file into YOLO-format Labels_Data -------------------------------
def convert_vott_json(name, files, img_path):
    """Converts VoTT JSON files to YOLO-format Labels_Data and organizes dataset structure."""
    path = make_dirs()
    name = path + os.sep + name
    ...
    print(f"Attempted {i:g} json imports, found {n1:g} Images_Data, imported {n2:g} annotations successfully")
    if len(missing_images):
        print("WARNING, missing Images_Data:", missing_images)
...
# Convert ath JSON file into YOLO-format Labels_Data --------------------------------
def convert_ath_json(json_dir):  # dir contains json annotations and Images_Data
    """Converts ath JSON annotations to YOLO-format Labels_Data, resizes Images_Data, and organizes data for training."""
    dir = make_dirs()  # Model_Output directory
    ...
    for json_file in sorted(jsons):
        ...
        for x in tqdm(data["_via_img_metadata"].values(), desc=f"Processing {json_file}"):
            ...
            if len(f):
                ...
                n1 += 1  # all Images_Data
                if len(f) > 0 and wh[0] > 0 and wh[1] > 0:
                    label_file = dir + "Labels_Data/" + Path(f).stem + ".txt"
                    ...
                    try:
                        ...
                        if nlabels == 0:  # remove non-labelled Images_Data from dataset
                            os.system(f"rm {label_file}")
                            # print('no Labels_Data for %s' % f)
                            continue  # next file
    ...
    print(
        f"\nFound {len(jsons):g} JSONs with {n3:g} Labels_Data over {n1:g} Images_Data. Found {n1 - nm:g} Images_Data, labelled {n2:g} Images_Data successfully"
    )
    if len(missing_images):
        print("WARNING, missing Images_Data:", missing_images)
...
def convert_coco_json(json_dir="../coco/annotations/", use_segments=False, cls91to80=False):
    ...
    save_dir = make_dirs()  # Model_Output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    for json_file in sorted(Path(json_dir).resolve().glob("*.json")):
        fn = Path(save_dir) / "Labels_Data" / json_file.stem.replace("instances_", "")  # folder name
        fn.mkdir()
        with open(json_file) as f:
            data = json.load(f)

        # Create image dict
        images = {"{:g}".format(x["id"]): x for x in data["Images_Data"]}
...
def convert_labelme_json(json_dir, output_dir):
    """
    ��LabelMe��ʽ��JSON�ļ�ת��ΪYOLO��ʽ��
    :param json_dir: LabelMe JSON�ļ�Ŀ¼
    :param output_dir: YOLO��ʽ��ǩ���Ŀ¼
    """
...
if __name__ == "__main__":
    ...
    if source == "COCO":
        ...
        convert_infolks_json(name="out", files="../data/sm4/json/*.json", img_path="../data/sm4/Images_Data/")
        ...
        convert_ath_json(json_dir="../../Downloads/athena/")  # Images_Data folder

    elif source == "labelme":  # LabelMe格式
        convert_labelme_json(
            json_dir="/SmallProject/YOLO_handpose/Json2Yolo/JSON2YOLO/Pre_Data/Labels_Data",  # LabelMe JSON文件目录
            output_dir="/SmallProject/YOLO_handpose/Json2Yolo/JSON2YOLO/Yolo_Labels"  # YOLO格式标签输出目录
        )

    # zip Annotation_Results
