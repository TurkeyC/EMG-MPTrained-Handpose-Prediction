...
def convert(file, zip=True):
    """Converts Labelbox JSON Labels_Data to YOLO format and saves them, with optional zipping."""
    names = []  # class names
    file = Path(file)
    ...
    for img in tqdm(data, desc=f"Converting {file}"):
        ...
        label_path = save_dir / "Labels_Data" / Path(img["External ID"]).with_suffix(".txt").name
        image_path = save_dir / "Images_Data" / img["External ID"]
    ...
    d = {
        ...
        "train": "Images_Data/train  # train Images_Data (relative to path) 128 Images_Data",
        "val": "Images_Data/val  # val Images_Data (relative to path) 128 Images_Data",
        "test": " # test Images_Data (optional)",
        "nc": len(names),
        "names": names,
    }  # dictionary
