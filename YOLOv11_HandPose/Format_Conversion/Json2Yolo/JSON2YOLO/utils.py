...
def make_dirs(dir="new_dir/"):
    """Creates a directory with subdirectories 'Labels_Data' and 'Images_Data', removing existing ones."""
    dir = Path(dir)
    if dir.exists():
        shutil.rmtree(dir)  # delete dir
    for p in dir, dir / "Labels_Data", dir / "Images_Data":
...
def image_folder2file(folder="Images_Data/"):  # from utils import *; image_folder2file()
    """Generates a txt file listing all Images_Data in a specified folder; usage: `image_folder2file('path/to/folder/')`."""
...
def add_coco_background(path="../data/sm4/", n=1000):  # from utils import *; add_coco_background()
    """
    Adds COCO dataset background Images_Data to a specified folder and lists them in outb.txt; usage:

    `add_coco_background('path/', 1000)`.
    """
    p = f"{path}background"
    if os.path.exists(p):
        shutil.rmtree(p)  # delete Model_Output folder
    os.makedirs(p)  # make new Model_Output folder

    # copy Images_Data
    for image in glob.glob("../coco/Images_Data/train2014/*.*")[:n]:
...
def flatten_recursive_folders(path="../../Downloads/data/sm4/"):  # from utils import *; flatten_recursive_folders()
    """Flattens nested folders in 'path/Images_Data' and 'path/json' into single 'images_flat' and 'json_flat'
    directories.
    """
    idir, _jdir = f"{path}Images_Data/", f"{path}json/"
    nidir, njdir = Path(f"{path}images_flat/"), Path(f"{path}json_flat/")
    n = 0

    # Create Model_Output folders
    ...
    for parent, dirs, files in os.walk(idir):
        for f in tqdm(files, desc=parent):
            ...
            if suffix.lower()[1:] in img_formats:
                ...
                image = parent / f
                json = Path(parent.replace("Images_Data", "json")) / str(f).replace(suffix, ".json")

                os.system(f"cp '{json}' '{json_new}'")
                os.system(f"cp '{image}' '{image_new}'")
                # cv2.imwrite(str(image_new), cv2.imread(str(image)))

    print(f"Flattening complete: {n:g} jsons and Images_Data")
